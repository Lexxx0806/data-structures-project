...
// --- LISTING TEMPLATES ---
// Updated to match server.py with all 58 templates
...
function generateRandomPoints(count, width, height) {
  ...
  for (let i = 0; i < count; i++) {
    const template =
      LISTING_TEMPLATES[Math.floor(Math.random() * LISTING_TEMPLATES.length)];
    points.push({
      id: `listing-${i}`,
...
function updateSidebar() {
  ...
  if (foundPoints.length > 0) {
    ...
    for (const point of foundPoints) {
      ...
      card.id = point.id; // Use the *first* photo (photos[0]) for the thumbnail

      card.innerHTML = `
                <img src="${point.photos[0]}" alt="${
        point.title
...
function openImageGallery(point) {
  ...
  modalThumbnails.innerHTML = ""; // Set the first image as the main one

  modalMainImage.src = point.photos[0]; // Create all the thumbnails

  point.photos.forEach((photoSrc, index) => {
    const thumb = document.createElement("img");
    thumb.src = photoSrc;
    thumb.alt = `${point.title} thumbnail ${index + 1}`;
    thumb.className = "thumbnail-img"; // Add "active" class to the first thumbnail
    ...
    thumb.addEventListener("click", () => {
      // Change main image
      modalMainImage.src = photoSrc; // Update "active" class
...
canvas.addEventListener("mouseup", (e) => {
  if (!isDragging) return;
  isDragging = false; // --- THIS IS THE "SLOW" SEARCH ---
  ...
  console.log(`Found ${foundPoints.length} points.`); // --- END OF SLOW SEARCH ---
  updateSidebar();
  draw();
});
